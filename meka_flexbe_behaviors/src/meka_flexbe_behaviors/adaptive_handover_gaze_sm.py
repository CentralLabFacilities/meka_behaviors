...
class adaptive_handover_gazeSM(Behavior):
	...
	def __init__(self):
		...
		self.add_parameter('stop_dist', 1.22)
	...
	def create(self):
		...
		with _sm_adaption_4:
			...
			OperatableStateMachine.add('wait',
										...
										transitions={'done': 'HandoverAdaptionExecPush'},
										autonomy={'done': Autonomy.Off})
		...
		with _sm_gazerobotneutral_6:
			...
			OperatableStateMachine.add('robotHand',
										GazeAtTargetReturn(target='face', duration_type='long', force=False, use_timeout=True),
			...
			OperatableStateMachine.add('neutral',
										GazeAtTarget(target='neutral', duration_type='long', force=True, use_timeout=True),
		...
		with _sm_adaptiongazeathuman_7:
			...
			OperatableStateMachine.add('handoverAdaption',
										HandoverAdaptionExec(command='trigger', topic='/do_adaption', reality_damp=0.80, terminate_dist_override=0.08, terminate_timeout_override=300, fixed_orientation=True, terminate=True, use_reference_trajectory=True, joint_speed_limit=0.15),
			...
			OperatableStateMachine.add('forceInMotion',
										ForceMonitor(force_threshold=10, force_topic='/force_helper', arm_topic='/force_helper/arm'),
		...
		# x:30 y:365, x:130 y:365, x:348 y:373, x:247 y:367, x:430 y:365, x:530 y:365, x:630 y:365
		...
		with _sm_pushinhandgazeathuman_8:
			...
			OperatableStateMachine.add('forceDuringMovement',
										ForceMonitor(force_threshold=8, force_topic='/force_helper', arm_topic='/force_helper/arm'),
			...
			# x:688 y:139
			OperatableStateMachine.add('checkHandNear',
										CheckHandNearness(dist_threshold=0.04, n_steps=20, target_frame='handover_frame_right'),
		...
		with _sm_togglehandgaze_9:
			...
			# x:333 y:163
			OperatableStateMachine.add('robotHand',
										GazeAtTarget(target='robot_right_hand', duration_type='short', force=False, use_timeout=False),
		...
		with _sm_adjusttorso_10:
			...
			OperatableStateMachine.add('gaze@face',
										GazeAtTarget(target='face', duration_type='long', force=False, use_timeout=False),
										transitions={'target_not_found': 'finished'},
										autonomy={'target_not_found': Autonomy.Off})
